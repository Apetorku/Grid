      ...
      {projects.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground">Start a project to begin messaging</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {projects.map((project) => (
            <Link key={project.id} href={`/client/projects/${project.id}?tab=communication`}>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3 flex-1">
                      <Avatar className="h-12 w-12">
                        ...
                        <AvatarFallback>{getInitials(project.developer?.full_name || '')}</AvatarFallback>
                      </Avatar>
                    ...
                    <div className="flex items-start gap-2 text-sm">
                      <p className="text-muted-foreground flex-1 truncate">
                        <span className="font-medium">
                          {project.lastMessage.sender_role === 'client' ? 'You' : project.developer?.full_name}:
                        </span>{' '}
